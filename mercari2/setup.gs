...
const SCHEMA = {
  Sales: {
    jp: ['商品名', '販売価格', '配送方法', 'サイズ区分', '手数料', '送料', '収入', '販売日', '備考'],
    en: ['itemName', 'price', 'shippingMethod', 'sizeCategory', 'fee', 'shippingCost', 'income', 'saleDate', 'note'],
    widths: [260, 100, 150, 180, 100, 100, 100, 120, 280],
  },
  ShippingMaster: {
    jp: ['配送方法', 'サイズ区分', '送料'],
...
function setupSales_(ss) {
  ...
  const samples = [
    ...
    ['サンプル商品B（Tシャツ）',  3500, CONFIG.METHOD_RAKURAKU, 'ネコポス',         new Date(), '値下げ交渉あり'],
    ['サンプル商品C（家電）',    12000, CONFIG.METHOD_RAKURAKU, '宅急便 80サイズ',  new Date(), '梱包材を別途購入'],
  ];
  ...
  const rows = samples.map(function(s) {
    ...
    return [itemName, price, method, size, fee, shipping, income, saleDate, note];
  });
