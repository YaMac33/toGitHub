...
function renderDaySection(group) {
  ...
  return `
    <section class="day-section">
      ...
      <div class="timeline">
        ${group.items.map((item, index, items) => renderTimelineItem(item, index, items)).join("")}
      </div>
    </section>
  `;
}
...
function renderTimelineItem(item, index, items) {
  ...
  const arrival = item.start_time || "--:--";
  const departure = item.end_time || "--:--";

  const previousItem = index > 0 ? items[index - 1] : null;
  const requiredTime = previousItem
    ? formatTravelDuration(previousItem.end_time, item.start_time)
    : "";

  const meta = [];

  if (requiredTime) {
    meta.push(`<span class="meta-chip">所要時間: ${escapeHtml(requiredTime)}</span>`);
  }

  if (item.move_method) {
    meta.push(`<span class="meta-chip">移動手段: ${escapeHtml(item.move_method)}</span>`);
  }
  ...
  return `
    <article class="timeline-item">
      <div class="time-block">
        <div class="time-row">${escapeHtml(`到着 ${arrival}`)}</div>
        <div class="time-row">${escapeHtml(`出発 ${departure}`)}</div>
      </div>
...
function formatTravelDuration(fromTime, toTime) {
