...
window.MEETING_DETAIL_VIEW = (function () {
  ...
  const ITEM_CATEGORY_ORDER = { GI: 1, SG: 2, CJ: 3, HG: 4 };
  ...
  function buildNormalItemGroups(classifiedItems) {
    const categoryKeys = ["GI", "SG", "CJ", "HG"];

    return categoryKeys.map(function (categoryKey) {
      const items = classifiedItems.filter(function (row) {
        return row.is_normal && row.meta.category_key === categoryKey;
      });

      if (!items.length) return null;

      const text = compressDisplayNumbers(items, categoryKey);
      if (!text) return null;

      let label = "";
      if (categoryKey === "GI") label = "議案";
      else if (categoryKey === "SG") label = "請願";
      else if (categoryKey === "CJ") label = "陳情";
      else if (categoryKey === "HG") label = "発議案";
  ...
  function buildExceptionItemGroups(classifiedItems) {
    const grouped = {};
    ...
    return PRIMARY_EXCEPTION_PRIORITY.map(function (tag) {
      ...
      return {
        label: group.label,
        text: group.items.map(function (row) {
          return row.meta.display_label;
        }).join("、")
      };
    }).filter(Boolean);
  }

  function buildMeetingItemsViewModel(classifiedItems) {
    return {
      normal_groups: buildNormalItemGroups(classifiedItems),
      exception_groups: buildExceptionItemGroups(classifiedItems)
    };
  }
  ...
  function buildDetail(meetingId) {
    ...
    const itemViewModel = buildMeetingItemsViewModel(itemClassifiedRows);
