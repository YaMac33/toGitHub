(() => {
  ...
  const STORAGE_KEY = "windowDutyApp2State";
  const SLOTS = ["A", "B", "C", "D"];
  ...
  function createTargetProfile(dates, activeStaff) {
    const baseTotal = (dates.length * SLOTS.length) / activeStaff.length;
    const slotTarget = dates.length / activeStaff.length;
  ...
  function scoreCandidate(staff, { dateInfo, slot, context, target }) {
    ...
    const targetTotal = target.totalTargets.get(staff.staff_id) || target.baseTotal;
  ...
  function evaluateSchedule(context, target) {
    ...
    context.counts.forEach((count, staffId) => {
      const targetTotal = target.totalTargets.get(staffId) || target.baseTotal;
      ...
      SLOTS.forEach((slot) => {
        score += Math.abs(slotCounts[slot] - target.slotTarget) * 8;
      });
    });
  ...
  function renderBalance() {
    if (state.schedule.length === 0) {
      elements.balanceTableBody.innerHTML = `<tr><td colspan="8" class="empty-cell">当番表を作成すると表示されます。</td></tr>`;
      return;
    }
    ...
    getActiveStaff().forEach((staff) => {
      ...
      row.innerHTML = `
        ...
        <td>${item.total}</td>
        ${SLOTS.map((slot) => `<td class="${item.slots[slot] <= 2 ? "balance-good" : "balance-warn"}">${item.slots[slot]}</td>`).join("")}
