...
function setupSpreadsheet() {
  ...
  // --- MealLog シート（食事メニュー明細） ---
  const mealSheet = ss.insertSheet('MealLog');
  mealSheet.getRange('A1:D1').setValues([['日付', '区分', '品名', 'kcal']]);
  mealSheet.setFrozenRows(1);
  mealSheet.setColumnWidth(1, 100);
  mealSheet.autoResizeColumns(2, 3);
