...
function loadFavorites() {
  const arr = loadJson(FAVORITES_KEY, []);
...
function getStatusBadgeHTML(state) {
  if (state === "wrong") {
    return '<span id="statusPill" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-700">不正解</span>';
  }
  if (state === "mastered") {
    return '<span id="statusPill" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-green-100 text-green-700">マスター</span>';
  }
...
function weightedPickGroup(groups, sourceQuestions) {
  const available = [];
  if (groups.unanswered.length > 0 && Number(rates.unanswered) > 0) available.push({ key: "unanswered", weight: Number(rates.unanswered) });
  if (groups.wrong.length > 0 && Number(rates.wrong) > 0) available.push({ key: "wrong", weight: Number(rates.wrong) });
  if (groups.mastered.length > 0 && Number(rates.mastered) > 0) available.push({ key: "mastered", weight: Number(rates.mastered) });

  if (available.length === 0) return sourceQuestions.length === 0 ? null : "all";
...
function maybeRestrictToFavorites(pool) {
  ...
  return boost > 0 && Math.random() * 100 < boost ? favPool : pool;
}
...
function pickRandomQuestion() {
  ...
  const selectedGroup = weightedPickGroup(groups, sourceQuestions);
  let pool = groups[selectedGroup] || sourceQuestions;
...
function clearAnswerState() {
  ...
  $("answerInput").value = "";
  $("answerInput").classList.remove("border-red-500", "ring-red-500", "focus:ring-red-500");
...
function renderQuestion() {
  ...
  if (!currentQuestion) {
    $("question").textContent = favoriteMode
      ? "お気に入り出題モードですが、お気に入りの問題がありません。"
      : "出題できる問題がありません。data.js を確認してください。";
  ...
  $("statusPill").outerHTML = getStatusBadgeHTML(state).replace("px-2.5 py-0.5", "px-3 py-1");
  renderFavoriteUI();
...
function ensureProgress(id) {
  if (!progress[id]) {
    progress[id] = { status: "unanswered", correctStreak: 0, attempts: 0 };
  }
  return progress[id];
}

function showAnswerResult({ correct, revealed = false }) {
  ...
  if (correct) {
    ...
    $("resultBox").innerHTML = '<div class="text-2xl font-bold">正解！</div>';
  } else {
    $("resultBox").className = "p-5 rounded-xl text-center border-2 bg-red-50 border-red-200 text-red-700 w-full";
    $("resultBox").innerHTML = `
      <div class="text-2xl font-bold mb-2">${revealed ? "答えを確認しました" : "不正解"}</div>
  ...
  if (currentQuestion.explanation || currentQuestion.link) {
    ...
    let html = currentQuestion.explanation ? `<div>${escapeHtml(currentQuestion.explanation)}</div>` : "";
    if (currentQuestion.link) {
      html += `<div class="mt-2"><a href="${escapeAttribute(currentQuestion.link)}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:underline font-medium inline-flex items-center gap-1">参考リンク</a></div>`;
    }
...
function submitAnswer() {
  ...
  const correct = isCorrectAnswer(input, answers);
  const p = ensureProgress(currentQuestion.id);
  p.attempts = (p.attempts || 0) + 1;

  if (correct) {
    p.correctStreak = (p.correctStreak || 0) + 1;
    if (p.status !== "wrong" || p.correctStreak >= 2) p.status = "mastered";
  } else {
    p.status = "wrong";
    p.correctStreak = 0;
  }
...
function revealAnswer() {
  ...
  updateStats();
  renderList();
  showAnswerResult({ correct: false, revealed: true });
}
...
function renderList() {
  ...
  const filtered = questions.filter((q) => {
    ...
    return haystack.includes(search.normalize("NFKC"));
  });
  ...
  for (const q of filtered) {
    ...
    const linkHtml = q.link ? `<a href="${escapeAttribute(q.link)}" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:text-blue-700 ml-1" title="参考リンク">リンク</a>` : "";
    const fav = isFavorite(q.id);

    tr.innerHTML = `
      <td class="px-4 py-3 whitespace-nowrap">${getStatusBadgeHTML(state)}</td>
      <td class="px-4 py-3 whitespace-nowrap">
        <button class="favorite-toggle text-xl ${fav ? "text-yellow-500" : "text-gray-300 hover:text-yellow-500"}" data-fav-id="${escapeAttribute(q.id)}" type="button" title="お気に入り">${fav ? "★" : "☆"}</button>
      </td>
      ...
      <td class="px-4 py-3 text-gray-500 text-sm hidden lg:table-cell max-w-xs truncate" title="${escapeAttribute(q.explanation || "")}">${escapeHtml(q.explanation || "")}${linkHtml}</td>
      <td class="px-4 py-3 text-center whitespace-nowrap">
        <button class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-100 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors shadow-sm open-btn" data-id="${escapeAttribute(q.id)}" type="button">開く</button>
      </td>
    `;
    ...
    tr.querySelector(".favorite-toggle").addEventListener("click", () => toggleFavorite(q.id));
    body.appendChild(tr);
  }
}
...
function showTab(tab) {
  ...
  for (const key in tabs) {
    tabs[key].className = key === tab
      ? "px-4 py-3 text-sm font-bold text-blue-600 border-b-2 border-blue-600 focus:outline-none whitespace-nowrap transition-colors"
      : "px-4 py-3 text-sm font-medium text-gray-500 hover:text-gray-800 border-b-2 border-transparent hover:border-gray-300 focus:outline-none whitespace-nowrap transition-colors";
  }
...
function resetProgress() {
  if (!confirm("学習履歴をリセットします。よろしいですか？")) return;
  progress = {};
  saveProgress();
  updateStats();
  renderList();
...
function setQuestionsData(json) {
  if (!Array.isArray(json)) {
    throw new Error("data.js の QUIZ_DATA の形式が不正です。配列で定義してください。");
  }
  ...
  questions = json
    ...
    .map((item, index) => {
      const id = String(item.id || `question-${index + 1}`).trim();
      return {
        id,
        question: String(item.question || "").trim(),
        answer: Array.isArray(item.answer)
          ? item.answer.map((v) => String(v).trim()).filter(Boolean)
          : [String(item.answer || "").trim()].filter(Boolean),
        explanation: String(item.explanation || "").trim(),
        link: item.link ? String(item.link).trim() : ""
      };
    })
    .filter((q) => {
      if (!q.id || !q.question || q.answer.length === 0 || seen.has(q.id)) return false;
      seen.add(q.id);
      return true;
    });
...
document.addEventListener("keydown", (e) => {
  if ($("viewQuiz").classList.contains("hidden") || e.isComposing) return;
  if (e.key === "Enter") {
    e.preventDefault();
    if (!isAnswered && !$("answerInput").disabled) submitAnswer();
    else if (isAnswered) nextQuestion();
  }
  if (!isAnswered && e.key === "Escape") {
    revealAnswer();
  }
});
