...
// ============================================================
// ダッシュボード（今日の状態カード / 目標進捗バー）
// ============================================================

// 記録がある最古の日の体重（進捗バーの起点）
function getStartWeight_() {
  const sheet = getDailyLogSheet_();
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return null;
  const values = sheet.getRange(2, 1, lastRow - 1, 4).getValues();
  const withWeight = values.filter(function (r) { return r[3]; });
  if (withWeight.length === 0) return null;
  withWeight.sort(function (a, b) { return dateToStr_(a[0]) < dateToStr_(b[0]) ? -1 : 1; });
  return { date: dateToStr_(withWeight[0][0]), weight_kg: withWeight[0][3] };
}

// 直近で体重が記録されている日
function getLatestWeight_() {
  const sheet = getDailyLogSheet_();
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return null;
  const values = sheet.getRange(2, 1, lastRow - 1, 4).getValues();
...
// 今日の状態カードと目標進捗バーに必要な情報を一度に返す
function getDashboardData() {
  const today = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  const profile = getProfile();
  const todayLog = getDailyLog(today);
  const start = getStartWeight_();
  const latest = getLatestWeight_();

  // 今日の体重が未記録なら直近体重で基礎代謝を推定する
  let bmr = null;
  const refWeight = (todayLog && todayLog.weight_kg) ? todayLog.weight_kg : (latest ? latest.weight_kg : null);
  if (refWeight) {
    bmr = Math.round(calcBMR_(profile.sex, profile.age, profile.height_cm, refWeight));
  }
...
function getMonthSummary(startDate, endDate) {
  const logs = getDailyLogs(startDate, endDate);
  if (logs.length === 0) return { count: 0 };

  const balances = logs.filter(function (l) { return l.balance_kcal !== null; });
  const weights = logs.filter(function (l) { return l.weight_kg; });
  const avgBalance = balances.length
    ? Math.round(balances.reduce(function (s, l) { return s + l.balance_kcal; }, 0) / balances.length)
    : null;

  let weightChange = null;
  if (weights.length >= 2) {
    weightChange = Math.round((weights[weights.length - 1].weight_kg - weights[0].weight_kg) * 10) / 10;
  }
