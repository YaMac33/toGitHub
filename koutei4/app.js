...
function setupHeaderToggle(elements) {
  ...
  if (!headerToggle || !headerSupplement) {
    return;
  }
...
function renderOverview(summary, container) {
  ...
  const cards = [
    ...
    {
      label: "移動距離",
      value: summary.distanceCount ? formatDistance(summary.totalDistance) : "未設定",
  ...
  container.innerHTML = cards
    .map((card, index) => {
      const panelId = `overview-panel-${index + 1}`;
      return `
        <section class="overview-card">
          ...
          >
            ...
            <span class="overview-toggle-text">表示</span>
          </button>
          <div id="${escapeHtml(panelId)}" class="overview-panel" hidden>
  ...
  setupOverviewToggles(container);
}

function setupOverviewToggles(container) {
  const toggles = Array.from(container.querySelectorAll(".overview-toggle"));

  toggles.forEach((toggle) => {
    const panelId = toggle.getAttribute("aria-controls");
    const panel = panelId ? document.getElementById(panelId) : null;
    const text = toggle.querySelector(".overview-toggle-text");
    ...
    toggle.addEventListener("click", () => {
      ...
      if (text) {
        text.textContent = nextExpanded ? "閉じる" : "表示";
      }
    });
  });
}
...
function setupSectionObserver(dayNav) {
  if (!dayNav || typeof IntersectionObserver !== "function") {
    return;
  }
...
function renderTimelineItem(item, dayIndex, itemIndex, prevItem, nextItem) {
  ...
  const printTimeLine = buildPrintTimeLine(item, isMove);
  const printDistance = isMove ? formatDistanceLabel(distance) : "";

  const infoChips = [
    duration ? buildInfoChip("所要時間", duration) : "",
    distance ? buildInfoChip("距離", printDistance || distance) : ""
  ]
    .filter(Boolean)
    .join("");
  ...
  return `
    <article class="timeline-item" style="--item-delay:${animationDelay}ms">
      ...
      <div class="card">
        ...
        <div class="print-card-body" aria-hidden="true">
          ${printTimeLine ? `<p class="print-time">${escapeHtml(printTimeLine)}</p>` : ""}
          <p class="print-title-line">[${escapeHtml(category)}] ${escapeHtml(title)}</p>
          ${item.detail && item.detail !== title
            ? `<p class="print-detail">${escapeHtml(item.detail)}</p>`
            : ""}
          ${printDistance ? `<p class="print-distance">距離 ${escapeHtml(printDistance)}</p>` : ""}
          ${duration ? `<p class="print-duration">所要時間 ${escapeHtml(duration)}</p>` : ""}
          ${item.note ? `<p class="print-note">備考 ${escapeHtml(item.note)}</p>` : ""}
        </div>
      </div>
    </article>
  `;
...
function buildPrintTimeLine(item, isMove) {
  if (isMove) {
    const parts = [];

    if (item.start_time) {
      parts.push(`出発 ${item.start_time}`);
    }

    if (item.end_time) {
      parts.push(`到着 ${item.end_time}`);
    }

    return parts.join(" / ");
  }

  return buildTimeRange(item.start_time, item.end_time);
}
...
function compareItinerary(a, b) {
  ...
  const dayCompare = a.day_no - b.day_no;
  if (dayCompare !== 0) {
    return dayCompare;
  }
...
function buildPeriodText(start, end) {
  ...
  if (start && !end) {
    return formatWarekiSeirekiDate(start);
  }

  if (!start && end) {
    return formatWarekiSeirekiDate(end);
  }
...
function setExpandableState(button, panel, expanded, label) {
  ...
  button.textContent = expanded ? `${label}を閉じる` : `${label}を表示`;
