...
function renderTimelineItem(item, dayIndex, itemIndex, prevItem, nextItem) {
  ...
  return `
    <article
      class="timeline-item ${isMove ? "timeline-item-move" : "timeline-item-place"}"
      style="--item-delay:${animationDelay}ms"
      data-itinerary-item
      data-date="${escapeHtml(item.date)}"
      data-start-time="${escapeHtml(item.start_time)}"
      data-end-time="${escapeHtml(item.end_time)}"
      data-next-start-time="${escapeHtml(nextItem?.start_time || "")}"
    >
      ...
      <div class="axis-block" aria-hidden="true">
        ...
        <span class="now-marker">
          ...
          <span class="now-marker-label">現在</span>
        </span>
      </div>
...
function setupCurrentTimeMarker() {
  updateCurrentTimeMarker();

  window.setInterval(updateCurrentTimeMarker, 60 * 1000);
...
function updateCurrentTimeMarker() {
  ...
  const items = Array.from(document.querySelectorAll("[data-itinerary-item]"));

  items.forEach((element) => {
    const date = safeText(element.dataset.date);
    const startTime = safeText(element.dataset.startTime);
    const endTime = safeText(element.dataset.endTime);
    const nextStartTime = safeText(element.dataset.nextStartTime);

    const isCurrent = isCurrentTimeItem({
      date,
      startTime,
      endTime,
      nextStartTime,
    ...
    element.classList.toggle("is-current-time", isCurrent);

    const label = element.querySelector(".now-marker-label");
    if (label && isCurrent) {
      label.textContent = `現在 ${now.time}`;
    }
  });

  const sections = Array.from(document.querySelectorAll(".day-section"));
  sections.forEach((section) => {
    const hasCurrent = Boolean(section.querySelector(".timeline-item.is-current-time"));
    section.classList.toggle("is-current-day", hasCurrent);
  });
}

function isCurrentTimeItem(params) {
  const { date, startTime, endTime, nextStartTime, now } = params;
  ...
  if (endMinutes == null) {
    endMinutes = parseTimeToMinutes(nextStartTime);
  }
