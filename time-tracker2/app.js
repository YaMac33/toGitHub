(function () {
  ...
  function cacheElements() {
    ...
    els.editContent = document.getElementById("editContent");
  ...
  function bindEvents() {
    ...
    els.editStartedAt.addEventListener("input", recalculateEditWorkMinutes);
    els.editEndedAt.addEventListener("input", recalculateEditWorkMinutes);
    els.editPauseMinutes.addEventListener("input", recalculateEditWorkMinutes);
    els.editStatus.addEventListener("change", recalculateEditWorkMinutes);
  ...
  function renderHistory(logs) {
    ...
    var rows = latest.map(function (log, index) {
      ...
      return '<tr class="' + (index >= 3 ? "collapsible-extra" : "") + '">' +
        ...
        "<td>" + escapeHtml(log.work_content || "") + "</td>" +
    ...
    els.historyList.innerHTML = '<div id="historyRecordsCollapse" class="collapsible-data is-collapsed"><table class="history-table">' +
      "<thead><tr><th>日付</th><th>時間帯</th><th>カテゴリ</th><th>内容</th><th>状態</th><th>実作業</th><th>停止</th><th>操作</th></tr></thead>" +
  ...
  function openEditPanel(logId) {
    ...
    els.editContent.value = log.work_content || "";
  ...
  function recalculateEditWorkMinutes() {
    ...
    if (!isFinite(started.getTime()) || !isFinite(ended.getTime()) || ended.getTime() < started.getTime()) {
      els.editWorkMinutes.value = "";
      return;
    }

    var totalMinutes = Math.max(0, Math.round((ended.getTime() - started.getTime()) / 60000));
    els.editWorkMinutes.value = String(Math.max(0, totalMinutes - pauseMinutes));
  }
  ...
  function saveEditedLog(event) {
    ...
    if (totalMinutes !== null && pauseMinutes > totalMinutes) {
      els.editError.textContent = "停止時間が総経過時間を超えています。";
      return;
    }
    var workMinutes = totalMinutes !== null ? Math.max(0, totalMinutes - pauseMinutes) : 0;

    log.work_date = formatDate(started);
    log.work_category = category;
    log.work_category_custom = category === "その他" ? customCategory : "";
    log.work_content = content;
  ...
  function exportCsv() {
    var logs = loadLogs();
    var columns = ["log_id", "work_date", "work_category", "work_category_custom", "work_content", "started_at", "ended_at", "work_minutes", "pause_minutes", "status"];
